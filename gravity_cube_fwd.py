...
def main(): 
    ...
    msh = mesh.create_box(MPI.COMM_WORLD, [[0.0, 0.0, 0.0], [1, 1, 1]], [4, 1, 1], mesh.CellType.hexahedron)
    ...
    I = Identity(3)
    F = variable(grad(u_) + I) # for the unloading case, we use the deformed configuration as the reference configuration
    C = variable(F.T * F)
    E = variable(0.5 * (C - I)) 
    ...
    dx = Measure("dx", msh)
    G = inner(S, D_E) * dx + inner((ln(J) - p_ / lmbda), p_t) * dx - inner(g*rho, -u_t[2])*dx
    ...
    grid.save("fwd_cube.vtu") 
    warped.save("fwd_cube_warped.vtu") 
